...
def small_model(**overrides) -> Borealis:
    config = {
        ...
        "consolidation_min_updates": 2,
        "consolidation_loss_threshold": 10.0,
        **overrides,
    }
...
def test_borealis_run_returns_causal_logits_and_fast_state():
    ...
    assert result.fast_state.updates == 3
    assert result.persistence.mode == "consolidate"


def test_fast_adaptation_changes_later_logits_without_changing_durable_parameters():
    model = small_model()
    tokens = torch.tensor([1, 2, 3, 4])
...
def test_fast_state_snapshot_round_trips_and_preserves_predictions():
    model = small_model()
    tokens = torch.tensor([1, 2, 3, 4])
    state = model.run(tokens).fast_state
