...
# Training + inference: needs torch, psycopg2, the architecture, source, benchmark, and platform packages
image = (
...
def serialize(request: Request) -> dict:
    ...
    try:
        ...
        from architectures._common.schema import ModelManifest
    except Exception as _e:
        tb = traceback.format_exc()
        raise RuntimeError(f"architecture contract import failed: {_e}\n\n{tb}") from _e
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Uncaught: {type(exc).__name__}: {exc}\n\n{tb}"
        )
...
def _serialize_source(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"model.py exec failed — {type(exc).__name__}: {exc}\n\n{tb}"
        )
    finally:
    ...
    for attr_name in dir(module):
        ...
        if (
            ...
            and issubclass(obj, ArchitectureModel)
            and obj is not ArchitectureModel
        ):
            model_cls = obj
            break
    ...
    if model_cls is not None:
        ...
        raise HTTPException(
            status_code=422,
            detail="model.py must define an ArchitectureModel subclass with .serialize() "
...
def _serialize_inner(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"R2 fetch failed — {type(exc).__name__}: {exc}\n\n{tb}"
        )
