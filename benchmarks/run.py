...
benchmarks/results/latest.json (also time-stamped copy in benchmarks/results/).
...
-----
    ...
    python benchmarks/run.py --benchmark SanityCheck

    # POST results to the Piro web app after running:
    python benchmarks/run.py --post-url https://piro-henna.vercel.app --post-token <session-token>
...
sys.path.insert(0, str(ROOT.parent))  # make top-level research packages importable

from benchmarks._common.base import Benchmark, BenchmarkResult  # noqa: E402
from benchmarks._common.models import GPTBaseline  # noqa: E402
from benchmarks.adaptive_compute import default as _adaptive_default  # noqa: E402
from benchmarks.ood_generalization import default as _ood_default  # noqa: E402

# ---------------------------------------------------------------------------
# Student model stub
# Replaced with the real model once architectures/student/model.py exists.
# ---------------------------------------------------------------------------
...
def _print_table(targets: list[tuple[str, list[dict[str, Any]]]]) -> None:
    ...
    header = f"{'Benchmark':<{col_w}}" + "".join(f"{n:>{score_w}}" for n in target_names)
    sep = "─" * len(header)
    print()
    print(sep)
    print(header)
...
def _print_details(targets: list[tuple[str, list[dict[str, Any]]]]) -> None:
    ...
    for i, bench_name in enumerate(bench_names):
        ...
        has_detail = any(r["metadata"] and r["metadata"] != {} for _, r in per_target)
        if not has_detail:
            continue
...
def main() -> None:
    ...
    benchmarks = [b for b in REGISTRY if b.name in filter_names] if filter_names else list(REGISTRY)
    if not benchmarks:
...
def _post_results(
    ...
    payload = json.dumps({"suiteRunId": suite_run_id, "ranAt": ran_at, "results": results}).encode()
