...
try {
  ...
  if (config.target_prefix !== EXPECTED_OLD_PREFIX) {
    throw new Error(`expected target_prefix ${JSON.stringify(EXPECTED_OLD_PREFIX)}, found ${JSON.stringify(config.target_prefix)}`);
  }
  ...
  const after = await sql<{ config_json: string | null }[]>`
