...
from sources.associative_recall import generate_associative_recall_dataset, make_memory_episode
...
def test_episode_serializes_ordered_piro_inputs_without_role_labels():
    ...
    assert [item["parts"][0]["type"] for item in record["inputs"]] == [
        "text"
    ...
    assert sum(len(packet.splitlines()) for packet in episode.distractor_packets) == len(
        episode.distractors
    )
