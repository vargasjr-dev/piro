...
const details: Record<DiagramKind, { title: string; subtitle: string }> = {
  ...
  output: {
    title: "OutputHead",
    subtitle: "The baseline output head reads runtime weights; a CTM variant may read recurrent state instead.",
  },
  shouldHalt: {
    title: "ShouldHalt",
  ...
  weights: {
    title: "Weights",
    subtitle: "A proposed baseline storage layout split across durable INT4 tensors, BF16 fast state, and runtime metadata.",
  },
  loadWeights: {
    title: "LoadWeights",
...
const explanations: Partial<Record<DiagramKind, { doing: string; why: string }>> = {
  ...
  output: { doing: "Reads the runtime weights after the full adaptation scan into the externally returned output.", why: "The baseline response should use the state learned from the complete observed stream." },
