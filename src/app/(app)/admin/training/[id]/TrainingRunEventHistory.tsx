import type { TrainingRunHistoryEvent } from "~/lib/training-run-events";
...
export function TrainingRunEventHistory({
  ...
  return (
    <section className="mt-8 rounded-2xl border border-amber-900/30 bg-[#100c0a] p-5">
      <div className="flex items-baseline justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-amber-200/50">
            The milestones recorded for this training run.
          </p>
        </div>
      ...
      {events.length === 0 ? (
        <p className="mt-5 text-sm text-amber-200/50">No events recorded yet.</p>
      ) : (
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {entry.step !== undefined ? (
                  <span className="text-xs text-amber-200/50">Step {entry.step}</span>
                ) : null}
                <time
