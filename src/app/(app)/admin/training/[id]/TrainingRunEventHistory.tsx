...
const EVENT_STYLES: Record<string, string> = {
  ...
  run_created: "border-amber-400/35 bg-amber-400/10 text-amber-200",
  started: "border-orange-400/35 bg-orange-400/10 text-orange-200",
  run_claimed: "border-orange-400/35 bg-orange-400/10 text-orange-200",
  checkpointed: "border-sky-400/35 bg-sky-400/10 text-sky-200",
  checkpoint_saved: "border-sky-400/35 bg-sky-400/10 text-sky-200",
  ...
  resume_requested: "border-violet-400/35 bg-violet-400/10 text-violet-200",
};
        ...
        <ol className="mt-5 space-y-4">
          {events.map((entry, index) => (
            <li
              key={
                entry.id ??
                `${entry.event}-${entry.observedAt ?? "unknown"}-${entry.step ?? index}`
              }
            ...
            >
              ...
              <span
                className={`absolute left-0 top-1.5 h-3.5 w-3.5 rounded-full border ${EVENT_STYLES[entry.event] ?? DEFAULT_EVENT_STYLE}`}
              />
              <div className="flex flex-wrap items-center gap-2">
                <span
                  className={`rounded-full border px-2.5 py-1 text-xs font-semibold capitalize ${EVENT_STYLES[entry.event] ?? DEFAULT_EVENT_STYLE}`}
                >
                  {entry.event}
                </span>
