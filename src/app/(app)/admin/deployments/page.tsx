...
function UserLink({
  ...
  return (
    <Link
      ...
      className="pointer-events-auto relative z-20 font-semibold text-amber-300 underline decoration-amber-700/60 underline-offset-2 transition hover:text-amber-100"
    >
      {username || name}
    </Link>
  );
          ...
          <div className="space-y-3">
            {deployments.map((item) => {
              const targetUser = item.targetUserId
                ? targetUsersById.get(item.targetUserId)
                : null;

              return (
                <article
                  key={item.id}
                  className="relative rounded-2xl border border-amber-900/25 bg-[#13100c] outline-none transition-colors hover:border-amber-700/40 focus-within:border-amber-700/40"
                >
                  <Link
                    href={`/models/${encodeURIComponent(item.modelName)}`}
                    aria-label={`Open ${item.modelName} sandbox`}
                    className="absolute inset-0 z-0 rounded-2xl outline-none focus-visible:ring-2 focus-visible:ring-orange-400/70"
                  />
          ...
          {HOSTED_MODELS.map((item) => (
            ...
            >
              <Link
                ...
                aria-label={`Open ${item.displayName} sandbox`}
                className="absolute inset-0 z-0 rounded-2xl outline-none focus-visible:ring-2 focus-visible:ring-orange-400/70"
              />
              <div className="relative z-10 pointer-events-none flex items-center justify-between gap-4 p-5">
                <h3 className="truncate text-sm font-semibold text-amber-50">
                  {item.displayName}
                </h3>
                <span className="shrink-0 rounded-full border border-sky-400/25 bg-sky-400/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-sky-300">
                  Hosted
                </span>
              </div>
            </article>
