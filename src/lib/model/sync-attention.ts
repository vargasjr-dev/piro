import { matVec, dot, softmax, flatten } from "./linalg";
 ...
 *
 * With a single embedding, softmax always yields [1.0] — the sync matrix
 * only affects the mix when multiple distinct embeddings are provided.
 *
...
export class SyncAttention {
  ...
  forward(syncMatrix: number[][], embeddings: number[][] | number[]): number[] {
    ...
    const scaleFactor = 1 / Math.sqrt(queryDim);
    ...
    const scores: number[] = seqs.map((emb) => {
      ...
      return dot(query, key) * scaleFactor;
    });
