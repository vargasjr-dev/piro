...
import { GEMMA_MODEL_ID, GEMMA_TARGET, makeGemmaAdapter } from "./gemma";

test("Gemma adapter sends text generation requests to the configured endpoint", async () => {
  const originalFetch = globalThis.fetch;
  const originalUrl = process.env.GEMMA_API_URL;
  const originalKey = process.env.GEMMA_API_KEY;
  process.env.GEMMA_API_URL = "https://gemma.test/";
  ...
  try {
    ...
    assert.equal(request?.url, "https://gemma.test/v1/completions");
    ...
    if (originalUrl === undefined) delete process.env.GEMMA_API_URL;
    else process.env.GEMMA_API_URL = originalUrl;
...
test("Gemma adapter replays ordered inputs as one growing conversation", async () => {
  ...
  try {
    ...
    globalThis.fetch = async (_input, init) => {
      ...
      return Response.json({
        choices: [{ text: prompts.length === 2 ? "value_014" : "ACK" }],
