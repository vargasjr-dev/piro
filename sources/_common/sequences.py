...
---
    ...
    uv run python -m sources._common.sequences --split train --n 5000 --seed 42
    uv run python -m sources._common.sequences --split test  --n 1000 --seed 42
...
def _make_label(sorted_sequence: list[int]) -> str:
    return _LABEL_TEMPLATE.format(sorted_sequence=" ".join(str(n) for n in sorted_sequence))
...
if __name__ == "__main__":
    ...
    samples = generate_sorting_dataset(
        n=args.n, length=args.length, seed=args.seed, split=args.split
    )
    for s in samples:
