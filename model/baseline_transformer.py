...
class BaselineTransformer(PiroModel):
    ...
    def serialize_graph(cls) -> Optional[ArchitectureGraph]:
        ...
        return ArchitectureGraph(
            nodes=[
                ...
                GraphNode(id="layers", type="attention", label="Transformer Layers", detail=f"{hp['n_layers']} × pre-norm attention"),
    ...
    def __init__(self, config: TransformerConfig | None = None, **kwargs: int) -> None:
        ...
        self.config = cfg
        self.layers = nn.ModuleList([_TransformerLayer(cfg.embed_dim, cfg.n_heads, cfg.ffn_dim) for _ in range(cfg.n_layers)])
