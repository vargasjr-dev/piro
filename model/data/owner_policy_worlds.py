...
def _eligible_actions(policy: OwnerPolicy, candidates: tuple[Action, ...]) -> list[tuple[int, Action]]:
...
def _option_line(slot: int, action: Action) -> str:
    classes = ",".join(sorted({INGREDIENT_CLASSES[ingredient] for ingredient in action.ingredients}))
    return (
...
def _case_packet(case: PolicyCase, *, choice: int | None = None, feedback: str | None = None) -> str:
