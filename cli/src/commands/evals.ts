...
export async function evalsList(): Promise<void> {
  ...
  if (!response.ok) fail(response.status, response.body, "evaluation listing failed");
  const evals = (response.body as { evals: Array<Record<string, unknown>> }).evals;
  if (evals.length === 0) {
    console.log("No evaluations found.");
    return;
  }
  console.log("ID  STATUS  BENCHMARK  QUEUED_AT  COMPLETED_AT  COST_USD  RUNTIME_MS");
...
export async function evalsGet(id: string): Promise<void> {
  const config = resolveConfig();
  const response = await piroFetch(config, `/api/evals/${encodeURIComponent(id)}`);
  if (!response.ok) fail(response.status, response.body, "evaluation lookup failed");
